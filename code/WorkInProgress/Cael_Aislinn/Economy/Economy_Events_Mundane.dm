...
/datum/event/mundane_news/announce()
	var/datum/trade_destination/affected_dest = pickweight(economy_controller.weighted_mundaneevent_locations)
	var/event_type = 0
...
/datum/event/trivial_news/announce()
	...
	//newMsg.is_admin_message = 1
	var/datum/trade_destination/affected_dest = pick(economy_controller.weighted_mundaneevent_locations)
	newMsg.body = pick(
