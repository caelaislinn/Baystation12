...
/obj/machinery/power/rust_core
	...
	var/field_strength = MIN_RUST_STR
	var/field_frequency = MIN_RUST_FREQ
	var/field_projection = MIN_RUST_PROJ
...
/obj/machinery/power/rust_core/New()
	...
	//locate nearby inducers
	for(var/obj/machinery/portable_atmospherics/rust_phaseinducer/P in range(RUST_PHASE_RANGE, src))
		phase_inducers.Add(P)
		P.owning_cores.Add(src)

/obj/machinery/power/rust_core/Del()

	for(var/obj/machinery/portable_atmospherics/rust_phaseinducer/P in phase_inducers)
		phase_inducers.Remove(P)
		P.owning_cores.Remove(src)
	..()
...
/obj/machinery/power/rust_core/interact(mob/user)
	...
	if(stat & NOPOWER)
		...
		user << "\red This device is locked. Swipe your ID card to unlock it"
		user << browse(null, "window=core_gen")
	else if(state != 2)
		switch(state)
			if(1)
				user << "\red [src] needs to be welded to the floor first."
			if(0)
				user << "\red [src] needs to be bolted to the floor first."
	else
		...
		dat += "<hr>"
		dat += "<b>Field strength:</b> [field_strength] T<br>"
		dat += "\
		...
		<a href='?src=\ref[src];str=10'>\[+++\]</a> \
		<br>"
		dat += "<b>Field frequency:</b> [field_frequency] GHz<br>"
		dat += "\
		...
		if(stat & NOPOWER)
			font_colour = "red"
		dat += "<b>Power required:</b> <font color=[font_colour]>[active_power_usage] W</font><br>"
...
/obj/machinery/power/rust_core/Topic(href, href_list)
	if(href_list["str"])
		...
		field_strength = min(max(field_strength + dif, MIN_RUST_STR), MAX_RUST_STR)
	...
	if(href_list["freq"])
		...
		field_frequency = min(max(field_frequency + dif, MIN_RUST_FREQ), MAX_RUST_FREQ)
		if(owned_field)
			owned_field.ChangeFieldFrequency(field_frequency)

	if(href_list["proj"])
		var/dif = text2num(href_list["proj"])
		field_projection = min(max(field_projection + dif, MIN_RUST_PROJ), MAX_RUST_PROJ)
		if(owned_field)
			owned_field.ChangeFieldProjection(field_projection)
...
/obj/machinery/power/rust_core/proc/Shutdown()
	...
	if(owned_field)
		icon_state = "core0"
		owned_field.DestroyField()
		del(owned_field)
	luminosity = 0
	use_power = 1

/obj/machinery/power/rust_core/proc/AddParticles(var/obj/effect/accelerated_particle/particle)
	if(owned_field)
		return owned_field.AddParticles(particle.particle_type, particle.additional_particles + 1)
